# OpenFusion Android — native Android launcher / Ludashi 4.1 runtime

This fork bypasses the Windows/Tauri OpenFusion launcher completely. Android handles OpenFusion login, session/cookie creation, server/version discovery and then launches `ffrunner.exe` directly through Ludashi's ARM64EC/FEX runtime. **WebView2 is not used.**

See `OPENFUSION_BUILD.md` for the exact launch flow and build notes.
