...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // OpenFusion edition: the app provisions its own container, so the generic
        // Winlator onboarding is not needed. ImageFS installation still happens below.
        ...
        // OpenFusion Android v0.3: use a native Android launcher instead of the
        // Windows Tauri/WebView2 launcher. The legacy Winlator UI remains below
        // as a recovery path if this preference is manually disabled.
        boolean openFusionNativeMode = sharedPreferences.getBoolean("openfusion_native_launcher", true);
        if (openFusionNativeMode) {
            setContentView(R.layout.openfusion_launcher_activity);
            OpenFusionNativeLauncher nativeLauncher = new OpenFusionNativeLauncher(this);
            if (!ImageFsInstaller.installIfNeeded(this, nativeLauncher::bind)) {
                nativeLauncher.bind();
            }
            return;
        }
        ...
        if (editInputControls) {
            ...
            if (!ImageFsInstaller.installIfNeeded(this, this::requestAppPermissions)) {
